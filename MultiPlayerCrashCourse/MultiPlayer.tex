...
\begin{itemize}
	...
	\end{itemize}
\end{itemize}
因此对于组件级别的挂接，需要设置组件的Replicates。代码如下：
\begin{code}[caption={在构造函数中}]
\end{code}
\subsection{最佳实践与性能提示}
虽然将 Component 设置为 Replicated 可以解决附件同步问题，但这会增加网络带宽开销（Overhead）。
\begin{itemize}\small
	\item 每个 Replicated Component 都会占用网络资源。
	\item 对于复杂的装备系统，通常不仅仅依赖 Component Replication，而是\emph{倾向于使用变量复制}（Replicated Variables）配合 \texttt{OnRep} 函数。即复制一个状态变量，当客户端收到变量更新时，在本地执行附加逻辑，从而节省带宽。
\end{itemize}


\section{变量复制 (Variable Replication)}

变量复制允许服务器上的变量变化自动同步到所有客户端。要实现这一点，必须遵循以下步骤和规则：

\subsection{实现变量复制的三个步骤}
确保拥有变量的 Actor 本身已开启复制（\texttt{bReplicates = true}），然后在 C++ 中执行以下操作：

\begin{enumerate}
	\item \textbf{重写 GetLifetimeReplicatedProps 函数}:
	在头文件中声明，在源文件中实现此虚函数。它是配置复制规则的地方。
	\begin{code}
virtual void GetLifetimeReplicatedProps(TArray<FLifetimeProperty>& OutLifetimeProps) const override;
	\end{code}
	
	\item \textbf{标记变量为 Replicated}:
	在变量定义的 \texttt{UPROPERTY} 宏中添加 \texttt{Replicated} 标识符。
	\begin{code}
UPROPERTY(Replicated)
float Armor;
	\end{code}
	
	\item \textbf{注册复制变量 (DOREPLIFETIME)}:
	在 \texttt{GetLifetimeReplicatedProps} 的实现中，使用 \ci{DOREPLIFETIME} 宏注册该变量。需包含头文件 \texttt{"Net/UnrealNetwork.h"}。
	\begin{code}
...
\subsection{重要原则}
\begin{itemize}
	\item \textbf{服务器权威}: 只有服务器上对 Replicated 变量的修改才会同步给客户端。如果客户端自己修改了该变量，这个修改只在本地生效，不会发给服务器或其他客户端，且随时可能被服务器的正确值覆盖。
	\item \textbf{单向同步}: 默认情况下，复制是单向的（Server $\rightarrow$ Client）。
	\item \textbf{Actor 复制前提}: 如果 Actor 自身的 \texttt{Replicates} 属性为 \texttt{false}，即使代码中正确配置了变量复制，变量也不会进行网络传输。
\end{itemize}

\subsection{最佳实践：使用接口解耦}
在处理交互逻辑（如拾取护甲）时，推荐使用 \textbf{Interface (接口)} 来避免硬编码依赖具体的 Actor 类（如 Character）。
\begin{itemize}
	\item 创建一个 Interface（如 \texttt{UMPPlayer}），定义 \texttt{GrantArmor} 等函数。
	\item Armor Actor 检测到重叠时，检查对方是否实现了接口，并调用接口函数。
	\item Character 实现接口，并在 \texttt{GrantArmor} 中修改自己的 Replicated 变量。
\end{itemize}
\end{document}